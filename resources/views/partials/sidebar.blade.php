...
<aside :class="sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out flex flex-col {{ $isOverlay ? '' : 'md:relative md:translate-x-0' }}">
    ...
    <div class="p-6 mb-2">
        ...
        {{-- Branch Switcher (super_owner only) --}}
        @if(auth()->check() && auth()->user()->isSuperOwner() && isset($allBranches))
        <div x-data="{ branchOpen: false }" class="relative mt-3">
            <button @click="branchOpen = !branchOpen" class="w-full flex items-center justify-between px-3 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 rounded-xl text-xs font-bold text-blue-700 hover:border-blue-200 transition-all">
                <span class="flex items-center">
                    <svg class="w-4 h-4 mr-2 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>
                    {{ session('current_branch_name', 'Pilih Cabang') }}
                </span>
                <svg class="w-4 h-4 transition-transform" :class="branchOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
            </button>
            <div x-show="branchOpen" @click.away="branchOpen = false" x-transition class="absolute left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
                <a href="?switch_branch=all" class="block px-4 py-2.5 text-xs font-semibold transition-colors {{ !session('current_branch_id') ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50' }}">
                    <span class="inline-block w-2 h-2 rounded-full mr-2 {{ !session('current_branch_id') ? 'bg-blue-500' : 'bg-gray-300' }}"></span>
                    Semua Cabang
                    <span class="text-gray-400 ml-1">(ALL)</span>
                </a>
                @foreach($allBranches as $branch)
                <a href="?switch_branch={{ $branch->id }}" class="block px-4 py-2.5 text-xs font-semibold transition-colors {{ session('current_branch_id') == $branch->id ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50' }}">
                    <span class="inline-block w-2 h-2 rounded-full mr-2 {{ session('current_branch_id') == $branch->id ? 'bg-blue-500' : 'bg-gray-300' }}"></span>
                    {{ $branch->name }}
                    <span class="text-gray-400 ml-1">({{ $branch->code }})</span>
                </a>
                @endforeach
            </div>
        </div>
        @elseif(auth()->check() && isset($currentBranchName))
        <div class="mt-3 px-3 py-2 bg-gray-50 border border-gray-100 rounded-xl">
            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Cabang</span>
            <p class="text-xs font-bold text-gray-700">{{ $currentBranchName }}</p>
        </div>
        @endif
    </div>
    ...
    <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('branches.index') }}" class="flex items-center px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-200 {{ request()->routeIs('branches.*') ? 'bg-smash-blue text-white shadow-md shadow-blue-200' : 'text-gray-500 hover:bg-blue-50 hover:text-smash-blue' }}">
            <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if(auth()->check() && (auth()->user()->isOwner() || auth()->user()->isCashier()))
    ...
    <div class="p-4 border-t border-gray-100">
        ...
        <div class="flex flex-col space-y-3 bg-gray-50 rounded-2xl border border-gray-100 shadow-sm p-3">
            ...
            @if(auth()->user()->isOwner() && request()->routeIs('pos.index'))
